...
export const setup = () => {
  ...
  const HUSKY_BIN = 'npx husky';
  const COMMAND_BIN = `npx ${COMMAND}`;
  ...
  try {
    [
      `${HUSKY_BIN} install`,
      `${HUSKY_BIN} add ${PRE_COMMIT} "${COMMAND_BIN} push | true && git add ${SETTINGS_PATH};"`,
      `git add ${PRE_COMMIT}`,
      `${HUSKY_BIN} add ${POST_MERGE} "${COMMAND_BIN} pull | true;"`,
      `git add ${POST_MERGE}`,
      `${HUSKY_BIN} add ${POST_CHECKOUT} "${COMMAND_BIN} pull | true;"`,
